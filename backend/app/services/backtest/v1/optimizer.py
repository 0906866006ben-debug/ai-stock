...
from backend.app.services.backtest.optimizer_config import (
    CategoryMetrics,
    GateConfig,
    MIN_TRADES_FOR_UNBOUNDED_PF,
    OptimizerConfig,
    SplitMetrics,
    TrialResult,
    UNBOUNDED_PF_SENTINEL,
    detect_overfit,
    gates_passed_count,
)
...
def sample_params_weighted(
    ...
    rng: random.Random,
    value_weights: Optional[dict[str, dict[Any, float]]] = None,
    *,
    exploration_prob: float = 0.20,
) -> dict[str, Any]:
    """Sample one parameter set from per-dimension value weights.

    `exploration_prob` deliberately keeps some uniform random sampling in the
    loop so the optimizer does not collapse onto an early lucky region.
    """
    params: dict[str, Any] = {}
    for key, values in search_space.items():
        if (
            not value_weights
            or key not in value_weights
            or rng.random() < exploration_prob
        ):
            params[key] = rng.choice(values)
            continue

        weights_by_value = value_weights[key]
        weights = [max(float(weights_by_value.get(v, 0.0)), 0.0) for v in values]
        if sum(weights) <= 0:
            params[key] = rng.choice(values)
            continue
...
def sample_unique_params(
    ...
    *,
    ...
    """Generate up to `count` unique parameter sets.

    Duplicate parameter sets waste full backtests once Claude narrows a search
    space. This helper treats `count` as a ceiling: if a tiny grid is exhausted,
    it returns fewer than requested rather than re-running identical trials.
    """
    if count <= 0:
        return []
    ...
    if grid_size <= len(seen):
        return []

    # For very small spaces, shuffle the grid directly to guarantee coverage.
    remaining_slots = grid_size - len(seen)
    if grid_size <= 5000 and remaining_slots <= count:
        combos = [p for p in grid_iterator(search_space) if params_hash(p) not in seen]
        rng.shuffle(combos)
        selected = combos[:count]
        seen.update(params_hash(p) for p in selected)
        return selected
    ...
    for _ in range(min(count, remaining_slots)):
        ...
        for _attempt in range(max_attempts_per_param):
            if value_weights:
                ...
                params = sample_params_random(search_space, rng)
        ...
        # If weighted sampling kept colliding, fall back to a shuffled grid scan
        # for small/medium spaces. For huge spaces, collisions after many attempts
        # are improbable and this branch simply stops cleanly.
...
def build_adaptive_value_weights(
    ...
    *,
    ...
    """Infer per-value sampling weights from the best completed trials.

    This is a lightweight TPE-style categorical sampler: values appearing in
    elite trials get extra probability mass, while every value keeps a prior so
    exploration remains possible. Hard-rejected validation samples are ignored
    when there are enough non-degenerate results to learn from.
    """
    weights: dict[str, dict[Any, float]] = {
        key: {value: float(prior_weight) for value in values}
        for key, values in search_space.items()
    }
    if not results:
        return weights
    ...
    stable = [
        r for r in results
        if r.validation.n_trades >= 3 and r.objective_score > -1999.0
        and not r.overfit_warning
    ]
    usable = stable or [
        r for r in results
        if r.validation.n_trades >= 3 and r.objective_score > -1999.0
    ]
    ranked = sorted(usable or results, key=lambda r: r.objective_score, reverse=True)
    if not ranked:
        return weights

    elite_count = max(min_elites, int(math.ceil(len(ranked) * elite_fraction)))
    elite_count = min(elite_count, len(ranked))
    elites = ranked[:elite_count]

    for rank, trial in enumerate(elites):
        credit = float(elite_count - rank)
        for key, value in trial.params.items():
            if key not in weights or value not in weights[key]:
                continue
...
def compute_objective(
    ...
    # PF is mathematically undefined when there are no losing trades. We store
    # that as 999.0 for reporting, but a tiny no-loss sample (e.g. 5 trades) is
    # not reliable enough to optimize toward.
    if (
        val.profit_factor >= UNBOUNDED_PF_SENTINEL
        and val.n_trades < MIN_TRADES_FOR_UNBOUNDED_PF
    ):
        return -2000.0
...
def _run_segment(
    *,
    ...
    # Phase 9.8 speedup: ReplaySummary.signal_rows lets us skip the load_signals DB read.
    ...
    # Phase 9.8 speedup: SimulationSummary.trade_rows lets us skip the load_trades DB read.
...
def _adaptive_batch_size(max_trials: int, n_workers: int) -> int:
    """Batch size for adaptive sampling.

    Small enough to learn several times per run, large enough to keep worker
    pools busy and avoid excessive scheduling overhead.
    """
    if max_trials <= 0:
        return 0
    return min(max_trials, max(4, max(8, n_workers * 3)))
...
def _build_worker_args(
    *,
    ...
    return [
        {
            "trial_id": first_trial_id + offset,
...
def _consume_trial_result(
    ...
    *,
    ...
    is_best = best is None or result.objective_score > best.objective_score
    if is_best:
        best = result
...
def _run_adaptive_trials(
    *,
    ...
    """Run trials in explore/exploit batches using adaptive value weights."""
    results: list[TrialResult] = []
    best: Optional[TrialResult] = None
    seen_hashes: set[str] = set()
    next_trial_id = 1
    batch_size = _adaptive_batch_size(config.max_trials, n_workers)
    ...
    try:
        while next_trial_id <= config.max_trials:
            remaining = config.max_trials - next_trial_id + 1
            if len(results) >= max(4, n_workers * 2):
                value_weights = build_adaptive_value_weights(results, search_space)
            else:
                value_weights = None

            params_batch = sample_unique_params(
                search_space,
                rng,
            ...
            next_trial_id += len(params_batch)
            if best is not None and best.success:
                logger.info("trial %d hit success criteria — stopping after adaptive batch", best.trial_id)
                break
    finally:
        if pool_owned and pool is not None:
            pool.close()
            pool.join()
...
def run_optimization(
    ...
    *,
    ...
    Phase 9.8 speedup: when `external_pool` is provided, reuse it across
    multiple calls (e.g. closed-loop iterations) so the worker-process
    OHLCV / feature caches persist across iters. Caller owns lifecycle.
    """
    ...
    if config.method == "grid" and grid_size > 5000:
        ...
        method = config.method
    if method not in {"random", "grid", "adaptive"}:
        logger.warning("Unknown optimizer method %r, falling back to random", method)
        method = "random"
    ...
    if method == "grid":
        ...
        iterable = sample_unique_params(search_space, rng, config.max_trials)
    ...
    # Phase 9.8 speedup: external_pool persists across closed-loop iters so
    # worker OHLCV cache + feature cache survive. Owner manages cleanup.
    if external_pool is not None:
        pool = external_pool
        pool_owned = False
    else:
        ...
        for result in pool.imap_unordered(_worker_run_trial, worker_args):
            ...
            if result.success:
                if pool_owned:
                    ...
                    break
                logger.info(
                    "trial %d hit success criteria — draining shared pool tasks before return",
                    result.trial_id,
                )
    finally:
        if pool_owned:
