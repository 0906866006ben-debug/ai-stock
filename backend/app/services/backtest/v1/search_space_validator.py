...
def validate(raw_text: str, bounds_config: dict[str, Any]) -> ValidationResult:
    ...
    reverse_mapping = {dotted: short for short, dotted in parameter_mapping.items()}

    for key, values in parsed["next_search_space"].items():
        raw_key = str(key).strip()
        if raw_key in allowed_params:
            canonical_key = raw_key
        elif raw_key in reverse_mapping:
            canonical_key = reverse_mapping[raw_key]
        else:
            result.dropped_keys.append(raw_key)
            result.warnings.append(f"non-whitelisted key dropped: {raw_key}")
            continue

        if not isinstance(values, list) or len(values) < MIN_LIST_LEN:
            result.dropped_keys.append(raw_key)
            result.warnings.append(f"key {raw_key}: list empty or invalid, dropped")
            continue

        # Truncate to max length
        if len(values) > MAX_LIST_LEN:
            values = values[:MAX_LIST_LEN]
            result.warnings.append(f"key {raw_key}: truncated to {MAX_LIST_LEN} values")

        # Apply bounds: core_invariants take precedence
        invariant = core_invariants.get(canonical_key)
        bounds = allowed_params.get(canonical_key, [-1e9, 1e9])
        if invariant:
        ...
        if out_of_bounds:
            result.warnings.append(f"key {raw_key}: dropped out-of-bounds values {out_of_bounds} (allowed [{lo},{hi}])")

        if len(clean_vals) < MIN_LIST_LEN:
            result.dropped_keys.append(raw_key)
            result.warnings.append(f"key {raw_key}: no in-bounds values, dropped")
            continue

        # Dedup & sort for stability
        clean_vals = sorted(set(clean_vals))
        cleaned_space[canonical_key] = clean_vals
